...
for PROTO in 0 1 2; do
  ...
  for OFFSET in $(seq 0 $BATCH $((TOTAL_IMAGES-1))); do
    python3 np_bnnmnist.py -M $TOTAL -I $PARTY_ID $PARTY_ADDRS -b $BATCH -o $OFFSET -d $PROTO >> $OUT
  done
...
for OFFSET in $(seq 0 $BATCH $((TOTAL_IMAGES-1))); do
  python3 np_bnnmnist.py -M $TOTAL -I $PARTY_ID $PARTY_ADDRS -b $BATCH -o $OFFSET --no-legendre >> $OUT
done
gsutil cp $OUT gs://$BUCKET/
